...
class NativeHighwayIndex(Mapping):
    ...
    def __contains__(self, node_id):
        position = int(numpy.searchsorted(self._node_records['node_id'], node_id))
        return (
            position < len(self._node_records)
            and int(self._node_records['node_id'][position]) == node_id
        )
    ...
    def __getitem__(self, node_id):
        ...
        if (
            position >= len(self._node_records)
            or int(self._node_records['node_id'][position]) != node_id
        ):
