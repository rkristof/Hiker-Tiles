...
ways, level 2 adds highways sharing nodes with those ways, and level 3 adds the
next highway level. Collector always uses three levels. Fixed binary layout keeps
selected geometry in original PBF way order. Callers may provide excluded way IDs
after adjacency selection when selected geometry is already available from another
source.
