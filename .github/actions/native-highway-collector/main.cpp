...
struct OutputAccessSummary {
    Id relation_id;
    Id node_id;
    std::uint64_t external_way_count;
    std::uint32_t high_highway_count;
    std::uint32_t reserved;
};

static_assert(sizeof(OutputAccessSummary) == 32, "OutputAccessSummary format must remain 32 bytes");
...
struct OutputCounts {
    ...
    std::uint64_t access_summary_count;
};
...
struct AccessRequest {
    Id relation_id;
    std::unordered_set<Id> route_way_ids;
    std::vector<Id> candidate_node_ids;
};
...
std::vector<AccessRequest> read_access_requests(const std::string& filename) {
    std::ifstream input(filename);
    if (!input) {
        throw std::runtime_error("Unable to open access request file: " + filename);
    }

    std::uint64_t request_count = 0;
    if (!(input >> request_count)) {
        throw std::runtime_error("Invalid access request file: " + filename);
    }

    std::vector<AccessRequest> requests;
    requests.reserve(request_count);
    for (std::uint64_t request_index = 0; request_index < request_count; ++request_index) {
        Id relation_id = 0;
        std::uint64_t route_way_count = 0;
        std::uint64_t candidate_node_count = 0;
        if (!(input >> relation_id >> route_way_count >> candidate_node_count)) {
            throw std::runtime_error("Invalid access request header");
        }
        ...
        request.route_way_ids.reserve(route_way_count);
        request.candidate_node_ids.reserve(candidate_node_count);
        for (std::uint64_t index = 0; index < route_way_count; ++index) {
            Id way_id = 0;
            if (!(input >> way_id)) {
                throw std::runtime_error("Invalid access request way list");
            }
            request.route_way_ids.insert(way_id);
        }
        for (std::uint64_t index = 0; index < candidate_node_count; ++index) {
            Id node_id = 0;
            if (!(input >> node_id)) {
                throw std::runtime_error("Invalid access request node list");
            }
            request.candidate_node_ids.push_back(node_id);
        }
...
bool is_high_highway_type(const char* highway_type) {
    static const std::unordered_set<std::string> high_highway_types{
        "motorway",
        "trunk",
        "primary",
        "secondary",
    ...
    return high_highway_types.find(highway_type) != high_highway_types.end();
}
void write_output(
    const std::string& filename,
    OnePassCollector& collector,
    const std::vector<AccessRequest>& access_requests
) {
    ...
    std::unordered_set<Id> high_way_ids;
    high_way_ids.reserve(output_ways.size());
    for (const auto& way : output_ways) {
        if (is_high_highway_type(way.highway_type)) {
            high_way_ids.insert(way.id);
        }
    }

    std::unordered_map<Id, std::uint32_t> output_node_indexes;
    output_node_indexes.reserve(output_nodes.size());
    for (std::uint32_t node_index = 0; node_index < output_nodes.size(); ++node_index) {
        output_node_indexes.emplace(output_nodes[node_index].id, node_index);
    }

    std::vector<OutputAccessSummary> access_summaries;
    for (const auto& request : access_requests) {
        for (const Id node_id : request.candidate_node_ids) {
            const auto node_index = output_node_indexes.find(node_id);
            if (node_index == output_node_indexes.end()) {
                continue;
            }

            const auto& node = output_nodes[node_index->second];
            const auto start = static_cast<std::size_t>(node.edge_offset);
            const auto end = start + node.edge_count;
            std::unordered_set<Id> external_way_ids;
            external_way_ids.reserve(node.edge_count);
            std::uint32_t high_highway_count = 0;
            for (std::size_t edge_index = start; edge_index < end; ++edge_index) {
                const auto way_id = output_edges[edge_index].way_id;
                if (request.route_way_ids.find(way_id) != request.route_way_ids.end()
                    || !external_way_ids.insert(way_id).second) {
                    continue;
                }
                high_highway_count += high_way_ids.find(way_id) != high_way_ids.end();
            }
            if (!external_way_ids.empty()) {
                access_summaries.push_back({
                    request.relation_id,
                    node_id,
                    external_way_ids.size(),
                    high_highway_count,
                    0,
                });
            }
    ...
    write_block(output, access_summaries);
    if (!output) {
...
int main(int argc, char** argv) {
    if (argc != 9) {
        std::cerr << "Usage: native-highway-collector --input FILE --route-nodes FILE --route-requests FILE --output FILE\n";
        return 2;
    }
    ...
    try {
        ...
        std::string access_requests_filename;
        ...
        for (int index = 1; index < argc; index += 2) {
            ...
            } else if (option == "--route-requests") {
                access_requests_filename = value;
        ...
        if (input_filename.empty()
            || route_nodes_filename.empty()
            || access_requests_filename.empty()
            || output_filename.empty()) {
            throw std::invalid_argument("input, route nodes, route requests, and output are required");
        }
        auto route_node_ids = read_ids(route_nodes_filename);
        auto access_requests = read_access_requests(access_requests_filename);
        ...
        write_output(output_filename, collector, access_requests);
